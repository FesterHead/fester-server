...
- Added custom script in `servuo/custom-scripts/zerodowned/` ([`SOSDecoder.cs`](servuo/custom-scripts/zerodowned/SOSDecoder.cs), authored by zerodowned, https://github.com/zerodowned/Custom-Scripts-for-ServUO/blob/master/SOS%20Decoder/SOSDecoder.cs) providing an SOS Instant Transporter item (`[add SOSDecoder`) that teleports a player's vessel directly to open water near targeted SOS sunken treasure coordinates, with facet validation, safety checks, and unlimited uses (no charges required).
- Added [25-sos-decoder-vendor.patch](servuo/patches/25-sos-decoder-vendor.patch) and `SOSDecoderCost=10000` to [Vendors.cfg](servuo/Config/ServUO/Vendors.cfg) to add the SOS Instant Transporter (`SOSDecoder`) to NPC Fisherman shop inventories for 10,000 gold with runtime price parameterization.
- Added [RemoteBank.cs](servuo/custom-scripts/FesterUO/RemoteBank.cs) providing player command `[rbank` to open their personal bank container remotely from any location, with checks for death and criminal status.
- Added [24-tmap-book-vendor.patch](servuo/patches/24-tmap-book-vendor.patch), `TMapBookCost=1000`, and `TreasureMapDecoderCost=10000` to [Vendors.cfg](servuo/Config/ServUO/Vendors.cfg) to add both the Treasure Map & SOS storage book (`TMapBook`, 1,000 gp) and Treasure Map Instant Transporter (`TreasureMapDecoder`, 10,000 gp) to NPC Mapmaker shop inventories with runtime price parameterization.
...
### Changed
- Isolated item count reporting in `FestersResourceSatchel` ([ResourceSatchel.cs](servuo/custom-scripts/FesterUO/ResourceSatchel.cs)) so items inside the satchel do not count against the character's 125-item main backpack capacity (reporting only 1 item for the satchel itself), while accurately tracking the satchel's 1,000-item capacity internally and in tooltips.
