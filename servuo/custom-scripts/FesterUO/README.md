...
| [`RemoteBank.cs`](RemoteBank.cs) | Remote bank access command allowing players to open their personal bank container from any location, with safety checks for criminal status and death. | `[rbank` |
| [`ResourceSatchel.cs`](ResourceSatchel.cs) | Blessed, weight-free storage satchel (100% weight reduction) that isolates contained item count from the character backpack (reporting only 1 item for the satchel itself), automatically intercepts and stores gathered resources upon harvest, and safely holds recall runes. | Double-click to open |
...
| [`TillermanGump.cs`](TillermanGump.cs) | Compact, non-intrusive navigation gump for ship tillermen providing single-click controls for continuous sail, one-tile adjustments, turns, and emergency halt. | `[tillerman` |
| [`WildernessReagents.cs`](WildernessReagents.cs) | Automated procedural wilderness ground reagent spawning system across Trammel and Felucca. Spawns standard magery reagents (1–3 yield) tailored to terrain (Swamps, Beaches, Mountains, Forests) with configurable world cap, periodic replenishment, and world save persistence. | Periodic timer (15m default), or GM admin command `[WildReagents` (`status`, `respawn`, `clear`, `reload`) |
